...
<template>
  <div class="w-full h-full flex flex-col items-center justify-center p-4 md:p-8 overflow-hidden relative">
    ...
    <a v-if="designState.articleUrl && !isExportVideoProcess"
       :href="designState.articleUrl"
       target="_blank" 
       class="absolute top-6 right-6 z-[45] bg-zinc-900/80 hover:bg-zinc-800 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-2 group transition-all text-xs font-semibold text-white shadow-2xl hover:scale-105 active:scale-95"
    >
      <div class="p-1.5 bg-blue-500 rounded-full flex items-center justify-center">
         <ExternalLink class="w-3 h-3 text-white" />
      </div>
      <span>View News Source</span>
    </a>
